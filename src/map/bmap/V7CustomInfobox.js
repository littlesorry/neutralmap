...
define(["bmaps"], function() {
	...
	var CustomInfobox = function (map, options) {
	    ...
	    function _init() {
	        ...
	        _infoboxContainerId = "bing-map-infobox";
	    ...
	    function _createArrow(direction) {
	        var html = ["<div id='", _infoboxContainerId, "_arrow' style='position:relative;float:left;width:0;height:0;line-height:0;border-style:solid;z-index:9;"];
	    ...
	    function _render() {
	        ...
	        if (container != null && _anchor != null && _content != null) {
	            if (!_map.getBounds().contains(_anchor)) {  //hide if anchor is anchor latlong on the map
	                ...
	                //Determine the quadrant that the pushpin is in.
	                var key = (_map.getHeight() * 0.5 < pinPixel.y) ? 2 : 0;
	                // 1: content on right, 0: content on left
	                key += (_map.getWidth() * 0.66 < pinPixel.x) ? 1 : 0;

	                //Wrap contents
	                var contents = ["<div id='", _infoboxContainerId, "_content' style='position:relative;float:left;border:1px solid ", _options.borderColor, ";background-color:", _options.color, ";min-width:", _options.minWidth, "px;min-height:", _options.minHeight, "px;border-radius:", _options.borderRadius, "px;box-shadow:", _options.boxShadow,";'>", _content, "</div>"];
	                ...
	                if (_options.orientation) {
	                    ...
	                    if (key > 1) {  //bottom
	                    	...
	                    	/* remove offset when content on bottom */
	    ...
	    */
	    ...
	    this.getContent = function() {
	    	return $('#' + _infoboxContainerId + '_content');
	    };

	    this.getArrowXVector = function() {
	    	return $('#' + _infoboxContainerId + '_arrow').css("margin-left") === "0px";
	    };
	...
	window["CustomInfobox"] = CustomInfobox;
